		...
		try {
			...
			if (request.getServletPath().equalsIgnoreCase("/recovery")) {
				...
				if (request.getParameter("token").equals(request.getSession().getAttribute("token"))) {
					...
					new EnvioEmail().esqueciSenha(request.getServletContext().getRealPath("/") + File.separator + "img",
							"luispaulojr@gmail.com", "Sua nova senha ",
...
//					System.out.println(" senha é :" + cod);
			...
			if (request.getServletPath().equalsIgnoreCase("/solicita")) {
				...
				new EnvioEmail().esqueciSenha(request.getServletContext().getRealPath("/") + File.separator + "img",
						"luispaulojr@gmail.com", "Esqueci minha senha",
						"Esqueceu sua senha <a href='http://localhost:8080/projeto-l2p-hidemi2.3/recovery?token="
								+ token + "'>Clique aqui</a>");
